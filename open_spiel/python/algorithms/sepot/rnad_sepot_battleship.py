...
class RNaDSolver(policy_lib.Policy):
  ...
  def init(self):
    ...
    self.state_tensor_shape = self._game.state_tensor_shape()

    if self.config.state_representation == StateRepresentation.INFO_SET:
      self.iset_tensor_shape = self._game.information_state_tensor_shape()
    elif self.config.state_representation == StateRepresentation.OBSERVATION:
      self.iset_tensor_shape = self._game.observation_tensor_shape()
  ...
  def _state_as_env_step(self, state: pyspiel.State) -> EnvStep:
    ...
    return EnvStep(
        obs=np.transpose(np.array(obs, dtype=np.float64).reshape(self.iset_tensor_shape), (1, 2, 0)),
        state=np.transpose(np.array(state.state_tensor(), dtype=np.float64).reshape(self.state_tensor_shape), (1, 2, 0)),
