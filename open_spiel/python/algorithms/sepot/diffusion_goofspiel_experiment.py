...
from open_spiel.python.algorithms.sepot.discrete_diffusion import DiscreteDiffusion
...
parser.add_argument("--discrete", default=1, type=int, help="Are states generated only for a conditioned public state")
parser.add_argument("--encoder_decoder", default=0, type=int, help="Is the network trained with encoder-decoder for states")
...
# Training arguments
parser.add_argument("--training_sampling", default="all", type=str, help="Sampling method for training. Either 'all' for all states (only for small games), 'traj' for uniformly generated trajectories or 'traj_p' for trajectories sampled by a random policy")
...
parser.add_argument("--training_iterations", default=1001, type=int, help="Amount of training iterations")
parser.add_argument("--save_each", default=1000, type=int, help="Save the model each that many iterations")
...
parser.add_argument("--model_path", default="diffusion_models/goofspiel_5_descending/model_ns500_c1_ed0_cd128_ld256_hd256_s42_t1000000.pkl", type=str, help="Path to the model, if should be loaded fpr sampling")
...
def train():
  ...
  if args.discrete == 1:
    model = DiscreteDiffusion("goofspiel", game_params,
  ...
  for i in range(args.training_iterations):
    ...
    if i % args.save_each == 0:
      ...
      file_name = f"model_ns{args.noise_steps}_c{args.conditional}_d{args.discrete}_ed{args.encoder_decoder}_cd{args.conditional_dim}_ld{args.latent_dim}_hd{args.hidden_dim}_s{args.seed}_t{i}.pkl"
