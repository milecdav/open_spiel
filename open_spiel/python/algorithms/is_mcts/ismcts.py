...
from open_spiel.python.algorithms.reconstruct_public_state import reconstruct_states, ReconstructType
...
class ISMCTSBot(pyspiel.Bot):
  ...
  def run_search(self, state):
    ...
    root_states, _ = reconstruct_states(state, 0, reconstruct_type=ReconstructType.INFOSET, player=state.current_player())
    root_probs = []
    # print(len(root_states))
    for root_state in root_states:
      # iset_key = self.get_state_key(root_state)
      # assert iset_key == root_infostate_key
      temp_state = state.get_game().new_initial_state()
      prob = 1.0
  ...
  def run_simulation(self, state, is_root = False):
